...
void main(){
    ...
    vec3 light = normalize(light_pos - position);
    ...
    vec3 result = vec3(1.0) - exp(-light_color * exposure);
    result = pow(result, vec3(1.0 / gamma));
    result *= (1 / dist) * light_intensity + 0.03;
    out_color = vec4(result,length(result));
}
