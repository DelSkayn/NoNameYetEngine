...
Matrix4<T> & Matrix4<T>::toRotation(T x,T y,T z){
    ...
    rz[0][2] = T(0);   rx[1][2] = sin(x);  rx[2][2] = T(1);    rx[3][2] = T(0);
...
Matrix4<T> & Matrix4<T>::toRotation(const Vector3<T> & n,const Vector3<T> & v,const Vector3<T> & u){
    ...
    (*this)[0][1] = v.x(); (*this)[1][1] = v.y(); (*this)[2][1] = u.z(); (*this)[3][1] = T(0);
    (*this)[0][2] = n.x(); (*this)[1][2] = n.y(); (*this)[2][2] = u.z(); (*this)[3][2] = T(0);
...
Matrix4<T> & Matrix4<T>::toRotation(const Vector3<T> & forward, const Vector3<T> & up){
    ...
    Vector3<T> u = Vector3<T>(Vector3<T>::normalize(up)).cross(n);
    Vector3<T> v = n.cross(u);

    return toRotation(n,u,v);
}
...
Matrix4<T> & Matrix4<T>::toPerspective(T fov, T aspectRatio, T zNear,T zFar){
    ...
    (*this)[0][0] =  T(1)/(tanHalfFov * aspectRatio); (*this)[1][0] = T(0);            (*this)[2][0] = T(0);  (*this)[3][0] = T(1);
    (*this)[0][1] =  T(0);                            (*this)[1][1] = T(1)/tanHalfFov; (*this)[2][1] = T(0);  (*this)[3][1] = T(1);
    (*this)[0][2] =  T(0);                            (*this)[1][2] = T(0);            (*this)[2][2] = (-zNear - zFar)/zRange;  (*this)[3][2] = T(1);
    (*this)[0][3] =  T(0);                            (*this)[1][3] = T(0);            (*this)[2][3] = T(2)*zFar*zNear/zRange;  (*this)[3][3] = T(0);
...
Quaternion<T> Quaternion<T>::operator*(const Vector3<T> & other) const{
    const T w = ((*this)[1] * other[1]) - ((*this)[2] * other[2]) - ((*this)[3] * other[3]);
    const T x = ((*this)[0] * other[1]) + ((*this)[2] * other[3]) - ((*this)[3] * other[2]);
    const T y = ((*this)[0] * other[2]) + ((*this)[3] * other[1]) - ((*this)[1] * other[3]);
    const T z = ((*this)[0] * other[3]) + ((*this)[1] * other[2]) - ((*this)[2] * other[1]);
